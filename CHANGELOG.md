...
### Fixed
- **`PgBackRestArchiveStalled` queried a metric that does not exist, so it had never once evaluated its own condition.** The rule used `time() - pg_stat_archiver_last_archived_time_seconds`, but socialup-postgres-exporter's custom `queries.yaml` emits `pg_stat_archiver_last_archive_age` — a different name, and already an age rather than a Unix timestamp. The query matched nothing, which is NoData, which under `noDataState: Alerting` fires; the alert had been red continuously since it was provisioned on 2026-07-21 and would have stayed red no matter how healthy WAL archiving was. Switched to `(pg_stat_archiver_last_archive_age >= 0) or ((pg_stat_archiver_archived_count == 0) * 0 + 86400)`; the `>600` threshold carries over unchanged since both forms are an age in seconds. The fallback exists because the exporter reports NaN (not absence) when `pg_stat_archiver.last_archived_time` is NULL, and NaN fails every comparison — so a database that has *never* archived a WAL segment would otherwise keep degrading to NoData instead of alerting on the condition it is there to catch. That is not hypothetical: on the live instance `archived_count` and `failed_count` are both 0 and `last_archive_age` is NaN, i.e. archiving has never run. The sibling `PgBackRestArchiveFailuresOutpacingSuccesses` was unaffected and correctly reads Normal — it is the only rule in the file whose selectors happened to match what the exporter actually emits.
- **`google.golang.org/grpc` bumped v1.80.0 -> v1.82.1 for GO-2026-6061.** Vulnerabilities in the xDS RBAC authorization engine and the HTTP/2 transport server implementation. Indirect dependency (pulled in by `otlptracegrpc`), but `govulncheck` reports it as reachable from this module's own code — 11 call traces through `trace.Init` -> `trace.WithBatcher` and `trace.Provider.Shutdown` -> `TracerProvider.Shutdown` into `transport.*` — so it is a live call-path finding rather than a module-graph-only one. The advisory was published after the last CI run on `main`, so the `test` job began failing there without any code change. `go mod tidy` also pulled `golang.org/x/sys` v0.42.0 -> v0.43.0, `golang.org/x/text` v0.35.0 -> v0.36.0, and both `genproto/googleapis/{api,rpc}` stamps forward. No source changes required; `make test` and `make lint` pass.
- **RUM dashboard: two LogQL parse errors surfaced by the previous realign.** `Distinct sessions` applied `count by (session_id)` directly to a log stream, which LogQL rejects (`syntax error: unexpected )`) — aggregation needs a range vector inside. Now `count(sum by (session_id) (count_over_time({…} | logfmt | session_id != "" [$__range])))`. The three Core Web Vitals panels used `[$__rate_interval]`, which Grafana was not interpolating for these Loki timeseries, so Loki received the literal string (`not a valid duration string: "$__rate_interval"`); switched to `[$__interval]`. Both faults predated the realign — `Distinct sessions` previously showed a silent "No data" and the CWV panels had never rendered.
...
- `stack/grafana/provisioning/alerting/rules-scrape-health.yaml` — `SocialupExporterScrapeReturnsNoSamples`, alerting when any `socialup-*` job reports `scrape_samples_scraped < 1` for 10m. Added after `socialup-node-exporter` spent seven weeks answering scrapes with HTTP 200 and an empty body: `up` stayed 1 the whole time (it reports that a scrape succeeded and parsed, not that it contained anything), so the documented post-deploy check `up{job=~"socialup-.*"} == 1` in `stack/README.md` passed while every `node_filesystem_*` and `pgbackrest_*` series was absent. The seven rules reading that exporter all went to NoData and, via `noDataState: Alerting`, fired continuously — naming seven symptoms ("root filesystem over 85% full", "no full backup in over 8 days", a failing stanza check) and never the cause, each one a claim about data that did not exist. `noDataState: Alerting` was working as designed; it just cannot say more than "this rule has no data", once per rule. `scrape_samples_scraped` is used rather than `up` precisely because it is 0 in both the target-down and the target-up-but-empty case, and the second is the one `up` is blind to; `up{job=~"socialup-.*"}` then distinguishes them. Uses `classic_conditions` for consistency with the sibling rule files, which collapses all matching series into one alert instance — the alert names the fault, and the query in its description recovers which job in one step.
